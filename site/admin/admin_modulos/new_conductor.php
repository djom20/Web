<?php
	if (isset($_REQUEST['edit'])) {
		$sql='SELECT `usuario`.`nombre`, `usuario`.`apellido`,`usuario`.`nombre2`,`usuario`.`apellido2`,`usuario`.`email`,`usuario`.`celular`,`usuario`.`trabajo` FROM `usuario` WHERE `usuario`.`estado`=1 AND `usuario`.`id_tipo`= 4 AND `usuario`.`id`='.$_REQUEST['edit'];
...
<article class="module width_3_quarter">
	<form id="form_articulo" name="articulo" <?php if (isset($_REQUEST['edit'])){ echo 'action="../../php/actualizar_conductor.php"'; }else{ echo 'action="../../php/crear_conductor.php"'; } ?> method="POST">
		<header><h3>Nuevo Conductor</h3></header>
